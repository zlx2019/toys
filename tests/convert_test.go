...
func TestAnyToBytes(t *testing.T) {
	bytes, _ := converts.ToBytes(111111.12)
	fmt.Println(bytes)
}

func TestToJson(t *testing.T) {
	json, _ := converts.ToJson([]string{"小张", "小王", "小刘"})
	fmt.Println(json) //["小张","小王","小刘"]

	json, _ = converts.ToJson(map[string]any{"name": "小张", "age": 18})
	fmt.Println(json) //{"age":18,"name":"小张"}
}

func TestToReadOnlyChannel(t *testing.T) {
	channel := converts.ToReadOnlyChannel([]int{1, 2, 3, 4, 5})
...
func TestToMap(t *testing.T) {
	...
	uMap := converts.SliceToMap(us, func(u Users) (string, Users) {
		return u.Name, u
	})
...
func TestEncoderAndDecoder(t *testing.T) {
	...
	bytes, _ := converts.EncoderBytes(u)

	u2 := &Users{}
	converts.DecoderBytes(bytes, u2)
